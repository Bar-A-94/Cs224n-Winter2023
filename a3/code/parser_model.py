...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
                 hidden_size=500, n_classes=3, dropout_prob=0.5):
        ...
        self.embed_to_hidden_weight = nn.Parameter(
            nn.init.xavier_uniform_(torch.empty(n_features * self.embed_size, hidden_size)))
    ...
    def forward(self, w):
        """ Run the model forward.
            ...
            PyTorch Notes:
                ...
                - When you apply your nn.Module to an input tensor `w` this function is applied to the tensor.
                    ...
                    the `forward` function would call on `w` and the result would be stored in the `output` variable:
                        model = ParserModel()
        ...
        # (batch_size, n_features * embed_size) @ (n_features * embed_size, hidden_size) > (batch_size, hidden_size)
        h = F.relu(x @ self.embed_to_hidden_weight + self.embed_to_hidden_bias)
        # In addition, include a dropout layer
        h_drop = self.dropout(h)
        # (batch_size, hidden_size) @ (hidden_size, n_classes) > (batch_size, n_classes)
