...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        #  By using shallow copy we can access only the non-empty without changing the original objects
        unfinished_parses[:batch_size] = [p for p in minibatch if not (len(p.buffer) == 0 and len(p.stack) == 1)]
    #  The dependencies for each (now completed) parse in partial parses.
