...
class BertSelfAttention(nn.Module):
    ...
    def attention(self, key, query, value, attention_mask):
        ...
        S = query @ key.transpose(-2, -1)  # [bs, num_attention_heads, seq_len, seq_len]
        S = S / torch.sqrt(torch.tensor(self.attention_head_size))
        S = S + attention_mask
        S = F.softmax(S, dim=-1)

        V = S @ value  # [bs, num_attention_heads, seq_len, attention_head_size]
        attention_score = V.transpose(1, 2).reshape((bs, seq_len, -1))  # [bs, seq_len, hidden size]
...
class BertLayer(nn.Module):
    ...
    """
        attention_score = self.self_attention(hidden_states, attention_mask)
