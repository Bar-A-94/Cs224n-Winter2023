...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 1. Randomly truncate the document to a length no less than 4 characters,
        # and no more than int(self.block_size*7/8) characters.
        rand_len = random.randint(4, int(self.block_size*7/8) + 1)
        truncated_doc = doc[:rand_len]
        ...
        rand_len = random.randint(1, int(len(truncated_doc)/2))
        start_idx = random.randint(0, len(truncated_doc)-rand_len)
        prefix = truncated_doc[:start_idx]
        masked_content = truncated_doc[start_idx: start_idx+rand_len + 1]
        suffix = truncated_doc[start_idx+rand_len + 1:]
